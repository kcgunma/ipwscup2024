...
def main(a_prefix, b_prefix, c_prefix):
    ...
    # Takes a row and a df and returns the ham_dist of the df with the minimum hamming_distance for the row and its index.
    def my_ham_dist(row, df):
        ham_dist = df.apply(lambda df_row: hamming_distance(row, df_row), axis = 1)
        min_dist, min_index = ham_dist.min(), ham_dist.idxmin()

        return pd.Series([min_dist, min_index], index=['Dist', 'T_idx'])
    ...
    for b_index, b_row in b.iterrows():
        ...
        # Assign all the rating parts to A (df of basic attribute) with the value of b_row (for apply).
        tmp_a = a.copy()
        tmp_a[b_row.index] = b_row

        # For each row of tmp_a, take T and return the ham_dist of T that minimises hamming_distance and its index.
        a_dist_df = tmp_a.apply(lambda row: my_ham_dist(row, T), axis = 1)

        min_index = a_dist_df["Dist"].idxmin()
        T_row = T.iloc[a_dist_df.loc[min_index, "T_idx"]]
